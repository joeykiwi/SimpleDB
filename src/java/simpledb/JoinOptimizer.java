...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            // join.
        	double result = cost1 + (card1*cost2) + (card1*card2);
            return result;
        }
    }
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
    	...
    	//method: estimation based on a range of the field for equality; a fixed value 0.3 for range query
    	
        int card = 1; //used to store the result
        
        //declare useful variables
        int table1id = tableAliasToId.get(table1Alias);
        int table2id = tableAliasToId.get(table2Alias);
        DbFile table1 = Database.getCatalog().getDatabaseFile(table1id);
        DbFile table2 = Database.getCatalog().getDatabaseFile(table2id);
        String table1name = Database.getCatalog().getTableName(table1id);
        String table2name = Database.getCatalog().getTableName(table2id);
        int table1FieldIndex = table1.getTupleDesc().fieldNameToIndex(field1PureName);
        int table2FieldIndex = table2.getTupleDesc().fieldNameToIndex(field2PureName);
        TableStats table1stat = stats.get(table1name);
        TableStats table2stat = stats.get(table2name);

        //in case that a join attribute is a primary key
        if(t1pkey) {
        	return card2;
        } else if(t2pkey) {
        	return card1;
        ...
        //if a join attribute does not contain any primary key, there are two plans to estimate
		double avgSelT2 = table2stat.avgSelectivity(table2FieldIndex, joinOp);
		int avgCardT2 = table2stat.estimateTableCardinality(avgSelT2);
		int plan1 = card1 * avgCardT2;
		
		double avgSelT1 = table1stat.avgSelectivity(table1FieldIndex, joinOp);
		int avgCardT1 = table1stat.estimateTableCardinality(avgSelT1);
		int plan2 = card2 * avgCardT1;
    ...
    public Vector<LogicalJoinNode> orderJoins(
    	...
    	PlanCache planStored = new PlanCache();
    	
    	for (int size = 1; size <= joins.size(); size++) { // for each size    		
    		for (Set<LogicalJoinNode> joinSet: enumerateSubsets(joins, size)) { //for each subset of size 'size'
    			
    			// we need to maintain and store the best plan for each subset
    			CostCard bestPlan = new CostCard();
    			bestPlan.cost = Double.MAX_VALUE; //initialize bestPlan
    			
    			for (LogicalJoinNode innerJoinNode : joinSet) { //for each join node as the inner-most join node (right-most in a join tree)
    															//way to ensure left-deep plan
    				//get the best plan with innerJoinNode as the right-most join node and the rest as a subplan (the left subtree)
    				CostCard currentPlan = computeCostAndCardOfSubplan(stats, filterSelectivities, innerJoinNode, joinSet, bestPlan.cost, planStored);
    				if (currentPlan != null && currentPlan.cost < bestPlan.cost) {
    						bestPlan = currentPlan;
    				}
    			}
    			
    			// update the best plan for the current subset
    			planStored.addPlan(joinSet, bestPlan.cost, bestPlan.card, bestPlan.plan);
    			
    		} //end for enumerateSubsets
    	}
    	
    	// if explain flag is on, print
    	if (explain) {
    		printJoins(joins, planStored, stats, filterSelectivities);
    	}
    	
    	// now, extract the best plan order from the plans stored
    	Set<LogicalJoinNode> resultSet = new HashSet<LogicalJoinNode>();
    	for (LogicalJoinNode n : joins)
    		resultSet.add(n);
        Vector<LogicalJoinNode> bestPlanOrder = planStored.getOrder(resultSet);
        if(bestPlanOrder == null) {
        	//error
        	System.exit(1);
        }
    ...
    private CostCard computeCostAndCardOfSubplan(
        ...
        if (news.isEmpty()) { // base case -- both are base relations
        	...
        	//System.out.println("tableName: "+table1Name);
        	//System.out.println("STATS " + stats);
        	//System.out.println("TABLE STATS " + stats.get(table1Name));
        	
            prevBest = new Vector<LogicalJoinNode>();
            
            if(stats.get(table1Name)==null)
            	System.out.println("missing table name1:"+table1Name);
            if(stats.get(table2Name)==null)
            	System.out.println("missing table name2:"+table2Name);
